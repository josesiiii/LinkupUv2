...
const calcCompatibility = (
  ...
  // MISMA INSTITUCIÓN
  if (
    userA.institution?.toString() !==
    userB.institution?.toString()
  ) {
    return 0;
  }
  ...
  score +=
    (sharedInterests.length / maxInt)
    * 40;

  // OBJETIVOS COMPARTIDOS
  const sharedObj =
    userA.objectives.filter(o =>
  ...
  try {
    ...
    if (!q) {
      ...
      return res.status(400).json({
        message:
          "Debes enviar un término de búsqueda"
      });
...
// FEED CONTEXTUAL
export const feedUsuarios = async (
  req,
  res
) => {
